...
PATRON_MLA = re.compile(r"^MLA\d+$", re.I)

# Nombres de columna que aceptamos, en orden de preferencia.
COLS_CLAVE = ["sku", "codigo", "código", "mla", "publicacion", "publicación",
              "item", "item_id", "id"]
...
def detectar_columnas(df, operacion):
    """
    Adivina cual es la columna de SKU/MLA y cual la del valor.
    ...
    # valores tipo MLA123456 o que parezcan SKU.
    if col_clave is None:
        for c in df.columns:
            muestra = df[c].dropna().astype(str).head(30)
            if len(muestra) and (muestra.str.match(PATRON_MLA).mean() > 0.5):
                col_clave = c
                break
...
def _a_numero(valor):
    """
    Convierte a numero tolerando formatos de planilla argentina:
...
def simular(df, pubs, operacion, col_clave=None, col_valor=None):
    ...
    if not col_clave or not col_valor:
        raise ValueError(
            ...
            f"Necesito una de SKU/MLA y una de "
    ...
    for _, fila in df.iterrows():
        ...
        # La planilla puede traer el MLA directo en vez del SKU.
        if PATRON_MLA.match(clave_cruda):
            ...
            filas.append(_fila_cambio(clave_cruda, pub, valor, campo_actual,
                                      operacion, "Indicada por MLA directo."))
            continue
