...
se usa el extremo barato del rango — `max_discounted_price`, el descuento
minimo que acepta la campaña — y **no** `suggested_discounted_price`, que es
una sugerencia de ML y viene mucho mas abajo. En los demas el precio ya viene
cerrado.
...
def _precio_promo(p):
    ...
    Si ML ya lo fijo (`price`), es ese. Si no, es **`max_discounted_price`**:
    el precio mas alto que la campaña acepta, o sea el descuento MINIMO para
    entrar.

    NO se usa `suggested_discounted_price`, que es solo una sugerencia y viene
    bastante mas abajo: medido en la cuenta de Uruguay (ago-2026) el sugerido
    pedia 17,85% donde el minimo real era 7,85%. Usarlo hacia parecer que las
    promos dejaban menos plata de la que dejan, y descartaba promos sanas por
